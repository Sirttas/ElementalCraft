...
public class SpellCooldownItemDecorator implements IItemDecorator {
    @Override
    public boolean render(GuiGraphicsExtractor guiGraphics, Font font, ItemStack stack, int x, int y) {
        var minecraft = Minecraft.getInstance();
        var localplayer = minecraft.player;
        var cooldown = localplayer == null ? 0.0F : SpellTickHelper.getCooldown(localplayer, SpellHelper.getSpell(stack), minecraft.getDeltaTracker().getGameTimeDeltaPartialTick(true));
