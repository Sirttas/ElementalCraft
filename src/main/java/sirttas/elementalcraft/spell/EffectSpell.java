		...
		if (!effects.isEmpty()) {
			for (IForgeEffectInstance forgeEffect : effects) {
				...
				if (effectinstance.getDuration() > 20) {
					iformattabletextcomponent = new TranslationTextComponent("potion.withDuration", iformattabletextcomponent, EffectUtils.getPotionDurationString(effectinstance, 1));
				}

				tooltip.add(iformattabletextcomponent.mergeStyle(effect.getEffectType().getColor()));
			}
		}
		ItemEC.addAttributeMultimapToTooltip(tooltip, multimap, new TranslationTextComponent("tooltip.elementalcraft.spell_effect_on_use").mergeStyle(TextFormatting.DARK_PURPLE));
	}
