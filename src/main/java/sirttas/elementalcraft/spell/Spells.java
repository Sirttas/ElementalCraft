...
public class Spells {
	...
	public static final ResourceKey<Registry<Spell>> REGISTRY_KEY = ResourceKey.createRegistryKey(ElementalCraftApi.identifier(ECNames.SPELL));
	private static final DeferredRegister<Spell> DEFERRED_REGISTER = DeferredRegister.create(ElementalCraftApi.identifier(ECNames.SPELL), ElementalCraftApi.MODID);

	public static final Registry<Spell> REGISTRY = DEFERRED_REGISTER.makeRegistry(b -> b.sync(true).defaultKey(ElementalCraftApi.identifier("none")));
	...
	public static final DeferredHolder<Spell, EffectSpell> HEAL = register("heal", k -> new  EffectSpell(k, new MobEffectInstance(MobEffects.INSTANT_HEALTH, 1, 1)));
	public static final DeferredHolder<Spell, EffectSpell> SPEED = register("speed", k -> new  EffectSpell(k, new MobEffectInstance(MobEffects.SPEED, 2400, 1), new MobEffectInstance(MobEffects.HASTE, 2400)));
	...
	private static <T extends Spell> DeferredHolder<Spell, T> register(String name, Function<Holder<SpellProperties>, ? extends T> builder) {
		return DEFERRED_REGISTER.register(name, i -> {
			var key = IDataManager.createKey(ElementalCraft.SPELL_PROPERTIES_MANAGER_KEY, i);
			var properties = ElementalCraft.SPELL_PROPERTIES_MANAGER.getOrCreateHolder(key);
