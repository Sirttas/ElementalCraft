...
public class ItemEC extends Item implements IItemEC {
	...
	@OnlyIn(Dist.CLIENT)
	public static void addAttributeMultimapToTooltip(List<ITextComponent> tooltip, Multimap<String, AttributeModifier> multimap, ITextComponent title) {
		if (!multimap.isEmpty()) {
			...
			for (Entry<String, AttributeModifier> entry : multimap.entries()) {
				...
				if (d0 > 0.0D) {
					tooltip.add((new TranslationTextComponent("attribute.modifier.plus." + attributemodifier.getOperation().getId(), ItemStack.DECIMALFORMAT.format(d1),
							new TranslationTextComponent("attribute.name." + entry.getKey()))).applyTextStyle(TextFormatting.BLUE));
				} else if (d0 < 0.0D) {
					d1 = d1 * -1.0D;
					tooltip.add((new TranslationTextComponent("attribute.modifier.take." + attributemodifier.getOperation().getId(), ItemStack.DECIMALFORMAT.format(d1),
							new TranslationTextComponent("attribute.name." + entry.getKey()))).applyTextStyle(TextFormatting.RED));
				}
			}
		}
	}
