...
public class EntityClientHandler {
    ...
    private static void extractSpellRenderStates(Entity entity, EntityRenderState state) {
        ...
        var mainState = extractSingleSpellRenderState(spell, null, entity, state);
    ...
    private static <S extends SpellRenderState> S extractSingleSpellRenderState(Holder<Spell> holder, @Nullable AbstractSpellInstance instance, Entity caster, EntityRenderState entityRenderState) {
        ...
        SpellRenderer<S> renderer = SpellRenderers.get(holder);
        ...
        renderer.extractRenderState(state, holder, instance, caster, spell.getHand(caster), entityRenderState.partialTick, entityRenderState.lightCoords);
        return state;
    }
	...
	private static void submitSpellEffectsFirstPerson(AbstractClientPlayer player, ItemStack stack, InteractionHand hand, float partialTicks, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight) {
        ...
        if (localPlayer.isUsingItem() && localPlayer.getUsedItemHand() == hand && !stack.isEmpty()) {
            submitSingleSpellFirstPerson(spell, null, localPlayer, hand, partialTicks, poseStack, submitNodeCollector, cameraRenderState, packedLight);
        }
	...
	private static void submitSingleSpellFirstPerson(Holder<Spell> holder, @Nullable AbstractSpellInstance instance, LocalPlayer localPlayer, InteractionHand hand, float partialTicks, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState, int packedLight) {
		...
		var renderer = SpellRenderers.get(holder);
        ...
        renderer.extractRenderState(state, holder, instance, localPlayer, hand, partialTicks, packedLight);
