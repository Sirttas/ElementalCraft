...
public class TagHasRunePredicate implements IRunePredicate {
	...
	@ObjectHolder(ElementalCraftApi.MODID + ":" + NAME) public static final BlockPosPredicateType<TagHasRunePredicate> TYPE = null;
	...
	public boolean test(IRuneHandler handler) {
		if (tag == null) {
			tag = ElementalCraftApi.RUNE_TAGS.getTag(tagId);
		}
		return tag.getValues().stream().mapToInt(handler::getRuneCount).sum() >= count;
	}
