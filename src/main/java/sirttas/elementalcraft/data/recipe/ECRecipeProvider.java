...
public class ECRecipeProvider extends RecipeProvider {
	...
	protected void registerRecipes(Consumer<IFinishedRecipe> consumer) {
		...
		ShapedRecipeBuilder.shapedRecipe(ECItems.containedCrystal).key('g', Items.GOLD_NUGGET).key('c', ECItems.inertCrystal).patternLine("ggg").patternLine("gcg").patternLine("ggg")
				.addCriterion(HAS_INERTCRYSTAL, this.hasItem(ECItems.inertCrystal)).build(consumer);
		ShapedRecipeBuilder.shapedRecipe(ECItems.shrineBase).key('w', ECBlocks.whiteRock).key('c', ECItems.inertCrystal).key('p', ECBlocks.elementPipe).patternLine(" p ").patternLine("pcp")
				.patternLine("www").addCriterion(HAS_WHITEROCK, this.hasItem(ECBlocks.whiteRock)).build(consumer);
		...
		prepareWhitterockInstrumentRecipe(ECBlocks.pedestal).key('n', ECBlocks.infuser).patternLine("wnw").patternLine("ici").patternLine("www").build(consumer);
		prepareWhitterockInstrumentRecipe(ECBlocks.pureInfuser).key('n', ECBlocks.infuser).key('p', ECBlocks.pedestal).patternLine("ini").patternLine("cpc").patternLine("www").build(consumer);
		...
		BinderRecipeBuilder.binderRecipe(ECItems.lavaShrine, ElementType.FIRE).addIngredient(ECItems.shrineBase).addIngredient(ECItems.fireCrystal).addIngredient(ECItems.pureCrystal)
	...
	private void registerSlabsStairsWalls(Consumer<IFinishedRecipe> consumer) {
		for (Block block : ForgeRegistries.BLOCKS) {
			if (ElementalCraft.MODID.equals(block.getRegistryName().getNamespace()) && !exists(block) && (block instanceof SlabBlock || block instanceof StairsBlock || block instanceof WallBlock)) {
				...
				shaped.addCriterion("has_" + sourceName, this.hasItem(source)).build(consumer);
				SingleItemRecipeBuilder.stonecuttingRecipe(Ingredient.fromItems(source), block, block instanceof SlabBlock ? 2 : 1).addCriterion("has_" + sourceName, this.hasItem(source))
	...
	private ShapedRecipeBuilder prepareInstrumentRecipe(IItemProvider result, IItemProvider crystal, int count) {
		return ShapedRecipeBuilder.shapedRecipe(result, count).key('i', Items.IRON_INGOT).key('c', crystal).addCriterion("has_" + crystal.asItem().getRegistryName().getPath(), this.hasItem(crystal));
	}

	private ShapedRecipeBuilder prepareWhitterockInstrumentRecipe(IItemProvider result) {
		return prepareInstrumentRecipe(result, ECItems.containedCrystal, 1).key('w', ECBlocks.whiteRock).addCriterion(HAS_WHITEROCK, this.hasItem(ECBlocks.whiteRock));
	}

	private ShapedRecipeBuilder prepareWhitterockInstrumentRecipe(IItemProvider result, IItemProvider crystal) {
		return prepareInstrumentRecipe(result, crystal, 1).key('w', ECBlocks.whiteRock).addCriterion(HAS_WHITEROCK, this.hasItem(ECBlocks.whiteRock));
	}
}
