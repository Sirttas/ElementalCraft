...
public class MekanismInteraction {
	...
	private static ChemicalDissolutionRecipe buildDissolutionRecipe(ChemicalDissolutionRecipe sourceRecipe, Ingredient ingredient) {
		return new ChemicalDissolutionIRecipe(ElementalCraft.createRL(ElementalCraft.PURE_ORE_MANAGER.buildRecipeId(sourceRecipe.getId())), ItemStackIngredient.from(ingredient),
	...
	private static void addInjector(MekanismRecipeType<ItemStackGasToItemStackRecipe> recipeType, ItemStackGasToItemStackRecipeFactory factory) {
		ElementalCraft.PURE_ORE_MANAGER.addInjector(PureOreRecipeInjector.create(recipeType, (sourceRecipe, ingredient) -> factory.create(ElementalCraft.createRL(ElementalCraft.PURE_ORE_MANAGER
				.buildRecipeId(sourceRecipe.getId())), ItemStackIngredient.from(ingredient), sourceRecipe.getChemicalInput(), sourceRecipe.getOutput(ItemStack.EMPTY, GasStack.EMPTY)))
				.filter((recipe, stack) -> recipe.getItemInput().test(stack)));
	}
	...
	private static void addInjector(MekanismRecipeType<ItemStackToItemStackRecipe> recipeType, ItemStackToItemStackRecipeFactory factory) {
		ElementalCraft.PURE_ORE_MANAGER.addInjector(
				PureOreRecipeInjector.create(recipeType, (sourceRecipe, ingredient) -> factory.create(ElementalCraft.createRL(ElementalCraft.PURE_ORE_MANAGER.buildRecipeId(sourceRecipe.getId())),
						ItemStackIngredient.from(ingredient), sourceRecipe.getOutput(ItemStack.EMPTY))).filter((recipe, stack) -> recipe.test(stack)));
	}
