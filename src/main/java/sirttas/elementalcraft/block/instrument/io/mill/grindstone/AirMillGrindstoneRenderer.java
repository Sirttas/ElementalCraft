...
@OnlyIn(Dist.CLIENT)
public class AirMillGrindstoneRenderer implements IECRenderer<AirMillGrindstoneBlockEntity> {
	...
	public void render(@Nonnull AirMillGrindstoneBlockEntity te, float partialTicks, @Nonnull PoseStack matrixStack, @Nonnull MultiBufferSource buffer, int light, int overlay) {
		...
		if (!stack.isEmpty() || !stack2.isEmpty()) {
			...
			matrixStack.mulPose(getRotation(te.getBlockState().getValue(AirMillGrindstoneBlock.FACING)));
