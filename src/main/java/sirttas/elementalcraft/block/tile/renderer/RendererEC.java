...
public abstract class RendererEC<T extends TileEntity> extends TileEntityRenderer<T> {
	...
	public void renderRunes(MatrixStack matrixStack, IRenderTypeBuffer buffer, IRuneHandler handler, float tick, int light, int overlay) {
		...
		handler.getRunes().forEach(rune -> {
			matrixStack.rotate(Vector3f.YP.rotationDegrees(90 / ruenCount));
			matrixStack.push();
			matrixStack.translate(0.75F, 0F, 0F);
			matrixStack.rotate(Vector3f.YP.rotationDegrees(90));
			matrixStack.scale(1F / 64F, 1F / 64F, 1F / 64F);
			renderIcon(matrixStack, buffer, 0, 0, rune.getSprite(), 16, -16, light, overlay);
			matrixStack.pop();
		});
		matrixStack.pop();
	}

	public void renderIcon(MatrixStack matrixStack, IRenderTypeBuffer buffer, int x, int y, RenderMaterial renderMaterial, int width, int height, int light, int overlay) {
		...
		builder.pos(matrix, x, y, 0).color(1F, 1F, 1F, 1F).tex(sprite.getMinU(), sprite.getMinV()).overlay(overlay).lightmap(light).normal(normal, 0, 1, 0).endVertex();
		builder.pos(matrix, x + width, y, 0).color(1F, 1F, 1F, 1F).tex(sprite.getMaxU(), sprite.getMinV()).overlay(overlay).lightmap(light).normal(normal, 0, 1, 0).endVertex();
		builder.pos(matrix, x + width, y + height, 0).color(1F, 1F, 1F, 1F).tex(sprite.getMaxU(), sprite.getMaxV()).overlay(overlay).lightmap(light).normal(normal, 0, 1, 0).endVertex();
		builder.pos(matrix, x, y + height, 0).color(1F, 1F, 1F, 1F).tex(sprite.getMinU(), sprite.getMaxV()).overlay(overlay).lightmap(light).normal(normal, 0, 1, 0).endVertex();
