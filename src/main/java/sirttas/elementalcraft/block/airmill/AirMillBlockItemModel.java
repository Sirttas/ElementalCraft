...
public class AirMillBlockItemModel extends BakedModelWrapper<BakedModel> {
    ...
    public AirMillBlockItemModel(ModelIdentifier originalName, BakedModel original) {
        super(original);
        ModelIdentifier brokenModel = ModelIdentifier.standalone(Identifier.fromNamespaceAndPath(originalName.id().getNamespace(), "item/" + originalName.id().getPath() + "_broken"));
