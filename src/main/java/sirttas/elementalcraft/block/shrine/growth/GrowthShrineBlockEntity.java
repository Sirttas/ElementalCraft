...
public class GrowthShrineBlockEntity extends AbstractShrineBlockEntity {
	...
	private boolean canGrow(BlockPos pos) {
		...
		if (state.is(ECTags.Blocks.GROWTHSHRINE_BLACKLIST)) {
			return false;
		}
		...
		if (block instanceof BonemealableBlock growable) {
			return (growable.isValidBonemealTarget(level, pos, state, level.isClientSide) && (growable.isBonemealSuccess(level, level.random, pos, state) || this.hasUpgrade(ShrineUpgrades.BONELESS_GROWTH)))
