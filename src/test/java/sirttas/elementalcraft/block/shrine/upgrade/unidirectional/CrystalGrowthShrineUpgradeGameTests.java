...
public class CrystalGrowthShrineUpgradeGameTests {
    ...
    public static void should_growAmethystOnBuddingBlocks(GameTestHelper helper) {
        helper.startSequence().thenExecuteAfter(1, () -> {
            ShrineGameTestHelper.forcePeriods(helper, new BlockPos(1, 1, 3), 36);
        }).thenExecuteAfter(1, () -> {
            helper.assertBlockPresent(Blocks.AMETHYST_CLUSTER, 1, 1, 2);
            helper.assertBlockPresent(Blocks.AMETHYST_CLUSTER, 1, 1, 0);
            helper.assertBlockPresent(Blocks.AMETHYST_CLUSTER, 1, 2, 1);
            helper.assertBlockPresent(Blocks.AMETHYST_CLUSTER, 1, 0, 1);
            helper.assertBlockPresent(Blocks.AMETHYST_CLUSTER, 2, 1, 1);
            helper.assertBlockPresent(Blocks.AMETHYST_CLUSTER, 0, 1, 1);
        }).thenSucceed();
    }
}
