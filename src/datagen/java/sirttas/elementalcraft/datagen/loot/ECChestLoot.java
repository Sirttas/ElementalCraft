...
import sirttas.elementalcraft.loot.function.RandomSpellFunction;
import sirttas.elementalcraft.loot.function.SetRuneFunction;
...
public class ECChestLoot implements LootTableSubProvider {
	...
	public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> consumer) {
		...
		consumer.accept(createKey("altar/small_water"), createSmallAltar(ElementType.WATER));
		consumer.accept(createKey("altar/medium_water"), createMediumAltar(ElementType.WATER));
		consumer.accept(createKey("altar/small_air"), createSmallAltar(ElementType.AIR));
		consumer.accept(createKey("altar/medium_air"), createMediumAltar(ElementType.AIR));
		consumer.accept(createKey("altar/small_earth"), createSmallAltar(ElementType.EARTH));
		consumer.accept(createKey("altar/medium_earth"), createMediumAltar(ElementType.EARTH));
	}
	...
	private static LootTable.Builder createInject() {
		return LootTable.lootTable().withPool(createBase(UniformGenerator.between(0, 2))
				...
				.add(LootItem.lootTableItem(ECItems.SCROLL.get()).apply(RandomSpellFunction.builder()).setWeight(15)));
	}
	...
	private static LootPool.Builder createBase(NumberProvider range) {
		return LootPool.lootPool().name("main").setRolls(range)
				...
				.add(LootItem.lootTableItem(ECItems.EMPTY_RECEPTACLE.get()).apply(SetItemCountFunction.setCount(UniformGenerator.between(1, 4))).setWeight(2));
	}
	...
	private static LootPool.Builder createAdvancedPool(ElementType type) {
		return LootPool.lootPool().name("advanced").setRolls(UniformGenerator.between(1, 3))
				...
				.add(LootItem.lootTableItem(ECItems.PURE_CRYSTAL.get()).setWeight(2)); // TODO receptacle with traits
	}
