...
public class ShrineUpgradeProvider extends AbstractManagedDataBuilderProvider<ShrineUpgrade, ShrineUpgradeBuilder> {
	...
	private static final IBlockPosPredicate FILLING_PREDICATE = IBlockPosPredicate.match(ECBlocks.SPRING_SHRINE.get()).and(FacingBlockPredicate.DOWN)
			.or(IBlockPosPredicate.match(ECBlocks.MELTING_SHRINE.get()).and(FacingBlockPredicate.NORTH.or(FacingBlockPredicate.SOUTH).or(FacingBlockPredicate.WEST).or(FacingBlockPredicate.EAST)));
	...
	protected void collectBuilders(HolderLookup.Provider registries) {
		...
		advancedShrineUpgrade(ShrineUpgrades.OVERCLOCKED_ACCELERATION)
				.predicate(ACCELERATION_PREDICATE)
