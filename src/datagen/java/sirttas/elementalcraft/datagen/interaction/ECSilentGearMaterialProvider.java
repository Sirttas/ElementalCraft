...
public class ECSilentGearMaterialProvider extends MaterialsProvider {
    ...
    protected Collection<MaterialBuilder> getMaterials() {
        ...
        List<MaterialBuilder> values = Lists.newArrayList();

        createBuilder(values, ECItems.WHITE_ROCK, 1)
        ...
        createBuilder(values, ECItems.DRENCHED_IRON_INGOT, 2)
        ...
        createBuilder(values, ECItems.SWIFT_ALLOY_INGOT, 3)
                .categories(MaterialCategories.METAL)
        ...
        createBuilder(values, ECItems.FIREITE_INGOT, 4)
                .categories(MaterialCategories.METAL)
                .namePrefix(new TranslatableComponent("material.elementalcraft.fireite"))
                ...
                .stat(PartType.COATING, ItemStats.ENCHANTABILITY, 7, StatInstance.Operation.ADD)
                .trait(PartType.TIP, Const.Traits.MALLEABLE, 2)
        ...
        createBuilder(values, ECItems.HARDENED_HANDLE, 2)
    ...
    public static MaterialBuilder createBuilder(List<MaterialBuilder> list, ItemLike provider, int tiers) {
        Item item = provider.asItem();
        MaterialBuilder builder = new MaterialBuilder(item.getRegistryName(), tiers, item);
